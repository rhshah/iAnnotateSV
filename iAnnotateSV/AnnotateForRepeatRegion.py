...
def ReadRepeatFile(filename, verbose):
    if(verbose):
        log.info("iAnnotateSV::AnnotateForRepeatRegion: Reading & Storing Repeat TSV file as dictionary")
...
def AnnotateRepeatRegion(verbose, count, sv, rrDict):
    if(verbose):
        log.info("iAnnotateSV::AnnotateForRepeatRegion: Checking Entry %d in Repeat data", count)
    ...
    if(list_loc1 != "None"):  # Check if there are no keys with a particular chromosome
        ...
        if(verbose):
            log.info("iAnnotateSV::AnnotateForRepeatRegion: Chromosome %s is not there in the repeat dictionary",
                     sv_chr1)
