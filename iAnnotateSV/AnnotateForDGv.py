...
def ReadDGvFile(filename, verbose):
    if(verbose):
        log.info("iAnnotateSV::AnnotateForDGv: Reading & Storing DGV TSV file as dictionary")
...
def AnnotateDGv(verbose, count, sv, dgvDict):
    if(verbose):
        log.info("iAnnotateSV::AnnotateForDGv: Checking Entry %d in DGv data", count)
    ...
    if(list_loc1 != "None"):  # Check if there are no keys with a particular chromosome
        for loc in list_loc1:  # For each location in all values check the overlap
            ...
            if (dgv_pos1 <= sv_pos1 <= dgv_pos2):
                ...
                list_svloc1.append('-'.join(slicedData))
    else:
        if(verbose):
            log.warn("iAnnotateSV::AnnotateForDGv: Chromosome %s is not there in the DGv dictionary", sv_chr1)
    ...
    if (list_loc2 != "None"):
        for loc in list_loc2:
            ...
            if (dgv_pos1 <= sv_pos2 <= dgv_pos2):
                slicedData = getVar(data, [2, 8])
                list_svloc2.append('-'.join(slicedData))
    elif verbose:
        log.warn("iAnnotateSV::AnnotateForDGv: Chromosome %s is not there in the DGv dictionary", sv_chr2)
