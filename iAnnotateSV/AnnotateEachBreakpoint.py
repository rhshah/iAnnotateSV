...
def AnnotateEachBreakpoint(chromosome, position, strand, df, autoSelect):
    ...
    if transcriptIndex:
        coordData = pd.DataFrame(
            index=np.asarray(transcriptIndex),
            columns=["c", "d", "e", "d1", "d2", "e1", "e2", "f"],
        )
        ...
        for tindex in transcriptIndex:
            ...
            if df.iloc[tindex]["strand"] == "+":
                ...
                if position > df.iloc[tindex]["cdsEnd"]:
                    ...
                    d = None
            # In exonic region
            exonStarts = sum(1 for item in filter(None, df.iloc[tindex]["exonStarts"].split(",")))
            exonEnds = sum(1 for item in filter(None, df.iloc[tindex]["exonEnds"].split(",")))
            in_exon = None
            for k in range(len(exonStarts)):
                if int(exonStarts[k]) <= int(position) and int(exonEnds[k]) >= int(
                    position
                ):
                    in_exon = k + 1
                    # print in_exon
            ...
            for k in range(exonCount):
                if (
                    int(exonEnds[k]) < int(position)
                    and int(exonStarts[k + 1]) > position
                ):
                    if df.iloc[tindex]["strand"] == "+":
                        f = exonFrames[k + 1]
                    else:
        ...
        if autoSelect:
            (
                ...
                intronnum,
                intronframe,
            ) = ft.FindATranscript(coordData, df)
        else:
            (
                geneName,
                transcript,
                ...
                intronnum,
                intronframe,
            ) = ft.FindAllTranscripts(coordData, df)
    # For Intergenic calls
        ...
        for y in 1000.0 ** (np.arange(1, 4, 0.3)):
            ...
            if not cmpDB.empty:
                ...
                desc = (
                    "IGR: "
                    + hp.bp2str(distBefore[beforeIdx], 2)
                    + " before "
                    + geneName
                    + "("
                    + strandDirection
            ...
            if not cmpDA.empty:
                ...
                desc = (
                    "IGR: "
                    + hp.bp2str(distBefore[afterIdx], 2)
                    + " after "
                    + geneName
                    + "("
                    + strandDirection
